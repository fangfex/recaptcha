...
2. 组件的初始化接口，初始化接口只能通过key调用，并且只能通过后台调用。并且可以设置简单模式或严格模式，默认为简单模式。
...
mode | 1或2，默认为1简单模式 | 1为简单模式，2为严格模式

> 模式说明：验证算法有10项判断条件，简单模式下只要满足7项条件即可通过，严格模式要求10项条件全满足，包括10分钟内5次的限制，如果不满足10条，则会显示图片验证码。
